...
|----|-------|-------|--------|
| D1 | `ConnectorDrawing.tsx`, `SelectionOverlay.tsx`, `ShapePalette.tsx` | `(clientX - rect.left - t.x) / t.scale` duplicated in 3+ places — extract to `clientToSvgCoords()` util in `canvas/units.ts` | [ ] |
| D2 | `toolbar/ConnectorDefaults.tsx`, `properties/ConnectorStyleEditor.tsx` | `ARROW_OPTIONS` defined twice with same values | [ ] |
| D3 | `canvas/AlignmentToolbar.tsx` | `distributeH` and `distributeV` are identical logic on different axes — extract generic `distributeAlongAxis()` | [ ] |
